...
// Probability that an infected district infects each healthy neighbor (0-100)
const int SPREAD_CHANCE = 45;

// Called at the end of each turn.
// Each infected district has a SPREAD_CHANCE% chance to infect each healthy neighbor.
// This makes expansion slower and less predictable than the old 100% version.
    ...
    while (node != nullptr) {
        for (auto [nr, nc] : neighbors(node->row, node->col)) {
            if (grid[nr][nc].state == HEALTHY) {
                // Only infect with SPREAD_CHANCE% probability
                if ((rand() % 100) < SPREAD_CHANCE) {
...
void applyAction(const std::string& actionType, int row, int col) {
    if (actionType == "vaccinate") {
        ...
        if (grid[row][col].state == HEALTHY) {
            ...
            // Only healthy districts can be quarantined by the player,
            // so they are never in the infection chain — removed that call.
            riskTree.remove(grid[row][col].risk, row, col);
            std::cout << "[engine] District (" << row << "," << col
                      << ") quarantined.\n";
        }
...
void initGrid() {
    // Seed the random number generator with current time so each run is different
    ...
    // Seed infection: random interior district (avoids edges so spread has room)
    int seedRow = 1 + rand() % (GRID_SIZE - 2);  // range [1, 6]
    int seedCol = 1 + rand() % (GRID_SIZE - 2);  // range [1, 6]
    grid[seedRow][seedCol].state = INFECTED;
    infectionChain.append(seedRow, seedCol, 1);
    riskTree.remove(RISK_MAP[seedRow][seedCol], seedRow, seedCol);
...
int main() {
    ...
    // if the loop ended because of MAX_TURNS and the game is still ongoing, it's a loss
    std::string finalStatus = checkGameStatus();
    if (finalStatus == "ongoing") {
        std::cout << "\n[engine] Turn limit reached — GAME OVER\n";
        writeStateJSON(MAX_TURNS);
    }
